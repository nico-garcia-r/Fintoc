...
class Fintoc:
    ...
    def retrieveLink(self, link_token):
        url = f"https://api.fintoc.com/v1/links/{link_token}"
        r = requests.get(url, headers=self.headers)
        return r.json()


    def listInvoices(self, link_token):
        url = f"https://api.fintoc.com/v1/invoices?link_token={link_token}"
        r = requests.get(url, headers=self.headers)
        return r.json()


    def listAccounts(self, only_id, link_token):
        url = f"https://api.fintoc.com/v1/accounts?link_token={link_token}"
        r = requests.get(url, headers=self.headers)
    ...
    def retrieveAccount(self, account_id, link_token):
        url = f"https://api.fintoc.com/v1/accounts/{account_id}?link_token={link_token}"
        r = requests.get(url, headers=self.headers)
        return r.json()


    def listMovements(self, account_id, link_token, only_id):
        url = f"https://api.fintoc.com/v1/accounts/{account_id}/movements?link_token={link_token}"
        r = requests.get(url, headers=self.headers)
    ...
    def retrieveMovement(self, movement_id, account_id, link_token):
        url = f"https://api.fintoc.com/v1/accounts/{account_id}/movements/{movement_id}?link_token={link_token}"
        r = requests.get(url, headers=self.headers)
        return r.json()
